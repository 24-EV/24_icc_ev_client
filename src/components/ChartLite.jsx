...
// title: 차트 제목
function ChartLite({ data = [], dataKeys = [], colors = [], title = '' }) {
  ...
  useEffect(() => {
    ...
    let observer = new window.ResizeObserver(() => {
      if (!chartRef.current) return; // null 체크
  ...
  return (
    <div className={cardPanelStyles.cardPanel} style={{ margin: '1.2rem 0', width: '100%' }}>
      <div
        style={{
          margin: '0 0 0.5rem 0',
          display: 'flex',
      ...
      >
        <h2 style={{ margin: 0 }}>{title} (코인/주식 스타일)</h2>
      ...
      <div className={legendStyles.legendPanel} style={{ margin: '8px 0 0 0' }}>
